...
class TimingLearnableController(SignalController):
	def __init__(self, agents, buffers, metrics, model_paths = None, device='cuda', min_green_time=5, max_green_time=80, save_path=None):
		import os
	...
	def get_reward(self, tls_id):
		# Reward based on change (delta) in waiting time instead of absolute value
		current_wait = self.metrics.get_current_waiting_time_at_tls(tls_id)
		# Optional scaling to keep values moderate
		scale = 100.0
		reward = -current_wait / scale
		if not np.isfinite(reward):
			print(f"[ERROR] reward NaN/Inf for TLS {tls_id}: current_wait={current_wait}")
		#reward = float(np.clip(np.nan_to_num(reward, nan=0.0, posinf=1e6, neginf=-1e6), -1e4, 1e4))
		return reward

	def _sanitize_obs(self, obs):
		if not np.all(np.isfinite(obs)):
			print(f"[ERROR] obs NaN/Inf detected: {obs}")
		#obs = np.nan_to_num(obs, nan=0.0, posinf=1e6, neginf=-1e6)
		return obs
	
	def add_sample_to_buffer(self, tls_id):
		buffer = self.buffers[tls_id]
	...
	def fishish_path_and_get_batch(self, tls_id, current_time):
		...
		buffer = self.buffers[tls_id]
		last_value = agent.net.forward(torch.tensor(self.build_obs(tls_id, current_time), dtype=torch.float32, device=self.device).unsqueeze(0))[2].item()
		batch = agent.finish_path(last_value, buffer=buffer)
		return batch
	
	def update_agent(self, tls_id, batch):
		agent = self.agents[tls_id]
		agent.update(batch)
	...
	def apply_control(self, tls_id, current_time, **kwargs):
		...
		if current_time - turn_time >= duration + 3:
			...
			next_timing = float(np.nan_to_num(action, nan=self.min_green_time, posinf=self.max_green_time, neginf=self.min_green_time))
			next_timing = float(np.clip(next_timing, self.min_green_time, self.max_green_time))
			
			self.turn_time[tls_id] = current_time
			self.duration[tls_id] = next_timing
			...
			self.previous_sample[tls_id]['action'] = float(next_timing)
	...
	def save_models(self):
		for tls_id in FIXED_TIME_PLANS:
			...
			self.agents[tls_id].save_model(model_file)
